...
def rnn_backward(dh, cache):
    ...
    N, T, H = dh.shape
    D = cache[0][0].shape[1]
    dx, dh0, dWx, dWh, db, dprev_hi = np.zeros((N,T,D)), np.zeros((N,H)), np.zeros((D,H)), np.zeros((H,H)), np.zeros(H), np.zeros((N,H))
     
    for i in range(T-1,-1,-1):
        dx[:,i,:], dprev_hi, dWxi, dWhi, dbi = rnn_step_backward(dh[:,i,:]+dprev_hi, cache[i])
        dWx += dWxi
        dWh += dWhi
        db += dbi
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    next_h = o * tanNC
    cache = (x, prev_h, prev_c, tanNC, Wx, Wh, b, i, f, o, g)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    N ,H = dnext_h.shape
    x, prev_h, prev_c, tanNC, Wx, Wh, b, i, f, o, g = cache
...
def lstm_backward(dh, cache):
    ...
    for i in range(T-1,-1,-1):
        dx[:,i,:], dprev_hi, dprev_c, dWxi, dWhi, dbi = lstm_step_backward(dh[:,i,:]+dprev_hi, dprev_c, cache[i])
        dWx += dWxi
        dWh += dWhi
        db += dbi
